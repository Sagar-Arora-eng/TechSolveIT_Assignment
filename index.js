...
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll(".custmonDropDwon").forEach(function (dropdown) {
    dropdown.addEventListener("click", function (e) {
      ...
      const customDrp = this.nextElementSibling || this.children[0];
      if (customDrp) {
...
document.querySelectorAll(".basic_banner_read_btn").forEach(function (button) {
  button.addEventListener("click", function () {
    if (basic_banner_read_btn === false) {
      ...
      document.querySelector(".basic_banner_text").classList.remove("active");
...
document.querySelectorAll(".filterToggelBtn").forEach(function (button) {
  button.addEventListener("click", function () {
    document.querySelector(".filterToggel").classList.toggle("active");
  });
});

document.querySelectorAll(".hideCustomDrp").forEach(function (button) {
  button.addEventListener("click", function () {
...
document.addEventListener(
  ...
  function (e) {
    ...
    if (
      ...
      document
        .querySelectorAll('[class="customPopup active"]')
        .forEach(function (customPopup) {
...
document.querySelectorAll(".closeIcon").forEach(function (closeIcon) {
  closeIcon.addEventListener("click", function () {
    document.querySelectorAll(".customPopup").forEach(function (customPopup) {
      customPopup.classList.remove("active");
    });
...
document.querySelector(".mainTable").forEach(function (mainTable) {
  mainTable.addEventListener("scroll", function (e) {
    ...
    document
      ...
      .forEach(function (element) {
        element.classList.toggle("before", isScroll);
      });
  });
});

document
  .querySelector(".currenciesTable, .exchangeTable")
  .forEach(function (table) {
    table.addEventListener("scroll", function (e) {
      let isScroll = e.currentTarget.scrollLeft;
      document
        .querySelectorAll(".ListingTable th:nth-child(2), td:nth-child(2)")
        .forEach(function (element) {
          element.classList.toggle("before", isScroll);
        });
    });
...
document.querySelector(".historical-data").forEach(function (historicalData) {
  historicalData.addEventListener("scroll", function (e) {
    let isScroll = e.currentTarget.scrollLeft;
    document
      .querySelectorAll(".ListingTable th:nth-child(1), td:nth-child(1)")
      .forEach(function (element) {
        element.classList.toggle("before", isScroll);
      });
  });
...
document.querySelector(".financialTable").forEach(function (financialTable) {
  financialTable.addEventListener("scroll", function (e) {
    let isScroll = e.currentTarget.scrollLeft;
    document
      .querySelectorAll(".ListingTable th:nth-child(1), td:nth-child(1)")
      .forEach(function (element) {
...
document
  .querySelector(".header-for-mobile  .searchPopup")
  .forEach(function (searchPopup) {
...
document
  .querySelector(".searchpopupHider")
  .forEach(function (searchpopupHider) {
...
document.addEventListener("scroll", function () {
  ...
  document
    .querySelectorAll(".gototopIconDiv")
    .forEach(function (gototopIconDiv) {
      gototopIconDiv.style.display = scrolled > 200 ? "block" : "none";
    });
});

document.querySelectorAll(".gototopIconDiv").forEach(function (gototopIconDiv) {
  gototopIconDiv.addEventListener("click", function () {
...
document.querySelectorAll(".popupRemover").forEach(function (popupRemover) {
...
if (isHighLight) {
  isHighLight.addEventListener("click", function () {
    let bannerCards = document.querySelectorAll("Banner_cards");
...
function buildCurrencyList(required_char = "") {
  ...
  currencyJson.popular.forEach((data, index) => {
    if (
      required_char != "" &&
      data.name.toLowerCase().indexOf(required_char) === -1
    )
      return;
    ...
    console.log(locurl);
    html +=
      `<div class="flexCountry commonFlex itemscenter` +
      (CURRCURRENCY == data.ukey ? " active" : "") +
      `" onclick="top.location='${locurl}';">
                     <img src="` +
      (data.crypto == 1
        ? "/assets/currencies/32x32/" + data.id + ".png"
        : "/assets/fiats/" + data.symbol + ".svg") +
      `" alt="">
                     <div class="rightText"><span>${data.name}</span><br> <small class="textUpperCase">${data.symbol} ` +
      (data.csymbol != null && data.csymbol != "" && data.symbol != data.csymbol
        ? " - " + data.csymbol
        : "") +
      `</small></div>
                     </div>`;
...
function buildLanguageList(required_char = "") {
  ...
  languageJson.forEach((data, index) => {
    if (
      required_char != "" &&
      data.name_dest.toLowerCase().indexOf(required_char) === -1
    )
      return;

    html +=
      `<li class="commonFlex itemscenter"><a href="` +
      (data.code == "en" ? CURPATHCUR : "/" + data.code + CURPATHCUR) +
      `" class="commonFlex itemscenter contentBetween flexLink"><span class="title">${data.name_dest}</span> <img class="flag" src="/assets/langflags/` +
      data.code +
      `.svg" alt=""></a></li>`;
  });
...
document.querySelectorAll(".faqAcc .flexAccHead").forEach(function (element) {
  element.addEventListener("click", function () {
    let AccHeadObj = this;
    let parentSiblings = AccHeadObj.parentElement.parentElement.children;

    for (let sibling of parentSiblings) {
      if (sibling != AccHeadObj.parentElement) {
        Array.from(sibling.children).forEach(function (child) {
          child.classList.remove("active");
        });
      }
    }
    let feqDesc = AccHeadObj.closest(".faqAcc li").querySelector(".faqDesc");
    feqDesc.classList.add("active");
  });
});
// == === == faq scripts == == == //
...
function toggleCurrencyMoreData(elem) {
  ...
  var bottomListingElements = document.querySelectorAll(
    ".currencyDetail .bottomListing"
  );

  if (currentstate == alternate) {
    //    $(".currencyDetail .bottomListing").css("display", "none");
    //    elem.innerHTML = toggleCurrencyMoreDataState;
    bottomListingElements.forEach(function (element) {
